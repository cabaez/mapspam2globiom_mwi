...
$set gdx_input   input_5min_2010_MI02_MWI.gdx
$set gdx_output  output_5min_2010_MI02_MWI.gdx
...
equations
    ...
    obj_min_ir_slack     objective function: minimize sum of ir slack
    obj_min_adm_slack    objective function: minimize weighted sum of adm slack
    obj_min_cl_slack     objective function: minimize sum of cl slack
    obj_min_all_slack    objective function: minimize weighted sum of ir adm and ir slack
    obj_min_s_slack      objective function: minimize sum of rur_pop coefficient slack for s system
    sum_one(j)           sum of land allocation shares is 1
    adm_stat(k,s)  adm statistics constraint without slack
    adm_stat_slack(k,s)  adm statistics constraint with slack
    ir_cover(i)          irrigated crops constraint with slack
    ir_cover_slack(i)    irrigated crops constraint with slack
    cl_cover(i)          land cover constraint without slack
    cl_cover_slack(i)    land cover constraint with slack
    det_alloc(i,j)       allocate shares for crops for which spatial data is available
...
*
cl_cover(i)..
    (1/scalef)*sum(system_grid(i,j), alloc(i,j)*crop_area(j)) =l= cl(i);
...
*
ir_cover(i)..
    (1/scalef)*sum(j, alloc(i,j)*ir_crop(j)) =l= ir_area(i);
...
s_alloc_slack(i,j)$j_s(j)..
    alloc(i,j) =e= scalef*(rps_factor + small_area_weights(i,j)*(s_slack(i,j,'plus')- s_slack(i,j,'minus')))* rur_pop_share(i,j);
...
adm_area, cl crop_area, scalef, ir_crop, det_area, ir_area, score, rur_pop_share,
