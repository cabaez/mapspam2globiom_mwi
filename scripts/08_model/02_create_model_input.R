...
# Synergy cropland
cl <- readRDS(file.path(proc_path, glue("harmonized/cl_{grid_sel}_{year_sel}_{iso3c_sel}.rds")))

# Synergy irrigated area
cl_ir <- readRDS(file.path(proc_path, glue("harmonized/cl_ir_{grid_sel}_{year_sel}_{iso3c_sel}.rds")))

# rur_pop_s
rur_pop_share <- readRDS(file.path(proc_path, glue("harmonized/rur_pop_share_{grid_sel}_{year_sel}_{iso3c_sel}.rds")))
...
if(sum(adm_art$pa) != sum(pa$pa[pa$adm_level == 0])){
  message("Totals do not add up! Stop.")}
...
s_system_s <- pa_fs %>%
  mutate(crop_system = paste(crop, system, sep = "_")) %>%
  filter(pa != 0, adm_level == 0, system == "S") %>%
  ungroup() %>%
  dplyr::select(crop_system) %>%
  unique() 
...
############### SAVE ###############
# GDX
wgdx(file.path(model_path, glue("input_{grid_sel}_{year_sel}_{iso3c_sel}.gdx")),
     cl_gdx, adm_area_gdx, ir_crop_gdx, ir_area_gdx, crop_area_gdx,
     s_system_s_gdx,
     score_gdx,
     grid_s_gdx, crop_system_s_gdx, 
     adm_s_gdx, crop_crop_system_s_gdx, 
     adm_grid_s_gdx,
