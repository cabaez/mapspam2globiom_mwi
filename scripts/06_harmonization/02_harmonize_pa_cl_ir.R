...
pa_I_tot <- sum(pa_fs$pa[pa_fs$system == "I" & pa_fs$adm_level == 0], na.rm = T)
cl_ir <- cl5_df %>%
...
sum(cl_harm$cl, na.rm = T) > sum(pa$pa[pa$adm_level == 0], na.rm = T)
sum(cl_harm$cl, na.rm = T)
sum(pa$pa[pa$adm_level == 0], na.rm = T)

# Final, rename adm_code_sel to adm_code
adm_code_sel <- glue("adm{adm_sel}_code")
cl_harm <- cl_harm[c("gridID", adm_code_sel, "cl")]
names(cl_harm)[names(cl_harm) == adm_code_sel] <- "adm_code"
...
############### SAVE ###############
# cl_harm
temp_path <- file.path(proc_path, paste0("harmonized"))
dir.create(temp_path, recursive = T, showWarnings = F)
saveRDS(cl_harm, file.path(temp_path, glue("cl_{grid_sel}_{year_sel}_{iso3c_sel}.rds")))
writeRaster(cl_harm_r, file.path(proc_path, glue("maps/cropland/synergy_cropland_{grid_sel}_{year_sel}_{iso3c_sel}.tif")), overwrite = T)

# cl_ir_harm
saveRDS(cl_ir_harm, file.path(temp_path, glue("cl_ir_{grid_sel}_{year_sel}_{iso3c_sel}.rds")))
writeRaster(cl_ir_harm_r, file.path(proc_path, glue("maps/irrigation/synergy_ir_area_{grid_sel}_{year_sel}_{iso3c_sel}.tif")), overwrite = T)
