#'========================================================================================================================================
#' Project:  mapspam
#' Subject:  Code to create score database
...
############### PREPARE SCORE AT ADM LEVEL ###############
# Function to prepare score in line with solve_sel

prep_score <- function(adm_code_sel){
  
  message(glue("Prepare score for {adm_code_sel}"))
  ...
  # Synergy cropland
  cl_raw <- readRDS(file.path(proc_path, glue("harmonized/{adm_code_sel}/cl_{grid_sel}_{year_sel}_{adm_code_sel}_{iso3c_sel}.rds")))
  
  #Synergy irrigated area
  cl_ir_raw <- readRDS(file.path(proc_path, glue("harmonized/{adm_code_sel}/cl_ir_{grid_sel}_{year_sel}_{adm_code_sel}_{iso3c_sel}.rds")))
  ...
  crop_system <- pa_fs %>%
    filter(adm_code == adm_code_sel, pa != 0) %>%
...
walk(adm_code_list, prep_score)


############### CLEAN UP ###############
rm(acc, acc_r, acc_raw, adm, adm_code_crop_h, adm_code_crop_l, adm_code_crop_s, adm_r,
   cl_ir_raw, cl_raw, crop_system, grid, grid_df, pa_fs, pa_fs_raw, pa_raw, pa,
   pop_raw, pop_rural, pop_rural_r, price, priors_base, py, rev, rur_pop_share,
   score_df, score_h, score_i, score_l, spam_stat2crop, suit, urban_mask,
   crop_h, crop_i, crop_l, crop_s)
