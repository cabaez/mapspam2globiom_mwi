...
if(!file.exists(file.path(param$raw_path, "gmia/gmia_v5_aei_ha_crs.tif"))){
  writeRaster(gmia_raw, file.path(param$raw_path, "gmia/gmia_v5_aei_ha_crs.tif"), overwrite = T)
}
...
if(param$res == "30sec") {
  # Crop, making sure it also includes grid cells, which center is outside the polygon by adding snap = "out")
  ...
  # Save
  writeRaster(gmia_temp, file.path(param$spam_path, 
    glue("processed_data/maps/irrigated_area/gmia_temp_{param$year}_{param$iso3c}.tif")), overwrite = T)
  ...
  # Warp and mask
  gmia_temp <- align_rasters(unaligned = input, reference = grid, dstfile = output,
