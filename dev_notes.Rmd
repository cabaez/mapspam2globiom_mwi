...
- need to illustrate the case when no subnational statistics are available => GAMBIA. 


# Finish documentation and functions
- We only describe cropping intensity and farming systems data in the subnational statistics section. This is inconsistent as one still needs this data if the model is run at adm0 only (e.g. Gambia). Need to add these to the national statistics section on collecting data and have an additional section on how to process/prepate them (now in process subnational statistics).
- The same inconsistency exists when creating data for countries for which no adm 1 or 2 data is available. Where should raw CI and FS data be stored? In the subnational statistics folder? Same when alternative adm0 crop data is used instead of FAO. Should this be stored in the subnational statistics folder? maybe better to rename: country_ or national_statistics, or simply crop_statistics?
...
- Finish process_subnat_stat.rmd
- If adm_level = 0, pdf of maps is empty => add adm0 map.
...
- better way to use stopifnot: https://www.r-bloggers.com/mining-r-4-0-0-changelog-for-nuggets-of-gold-1-stopifnot/
...
- Use logger package to create a log! https://cran.r-project.org/web/packages/logger/index.html
